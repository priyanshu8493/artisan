...
import {
  ...
  CreditCard, History, StickyNote, Undo2, User,
} from "lucide-react";
...
export default function AdminOrderDetailPage() {
  ...
  const nextStatuses = ORDER_STATUSES.filter((s) => s !== order.status);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 print:max-w-none">
      ...
      <section className="overflow-hidden rounded-lg card-surface shadow-subtle">
        <h2 className="border-b border-[rgb(var(--line)/0.6)] px-4 py-3 text-sm font-semibold">Items ({order.items.length})</h2>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="rounded-lg card-surface p-4 shadow-subtle">
          ...
          {order.user && (
            <div className="mt-3 flex items-center gap-2 text-sm text-[rgb(var(--muted))]">
              <User className="h-4 w-4" /> {order.user.name} · {order.user.email}
            </div>
          )}
          {order.customerNote && (
          ...
          {canRefund && (
            <Button
              ...
              onClick={() => {
                if (confirm("Record a refund for this order?")) act.mutate({ op: "refund", reason: "Admin-initiated refund" });
              }}
            >
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-lg card-surface p-4 shadow-subtle print:hidden">
            ...
            <Textarea rows={3} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Staff-only note." maxLength={2000} className="mt-2" />
            <Button
              size="sm"
              className="mt-2"
